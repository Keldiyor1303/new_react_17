...
export default function YaqinlashmoqdaContent({ itemsPerPage }) {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosInstance.get(`mainPage/allNear/${localStorage.getItem('ids')}`, {
                headers: {
    ...
    const handlePageClick = async (e) => {
        try {
            const res = await axiosInstance.get(`mainPage/allNear/${JSON.parse(localStorage.getItem('ids'))}?page=${e.selected}`, {
                headers: {
                    Authorization: "Bearer " + currentUser
                }
            })
                                ...
                                <table>
                                    <thead>
                                        <tr>
                                            ...
                                            <th style={{ width: '350px' }}>
                                                <div className="form-group form-group-feedback form-group-feedback-left inp buttonsinput">
                                    ...
                                    <tbody>
                                        {data?.content?.map((tableItem, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="text-center id" style={{ wordWrap: "break-word" }}>{tableItem?.correspondentName}</td>
                                                <td className="text-center id" style={{ wordWrap: "break-word" }}>{tableItem?.shortDescription}</td>
                                                ...
                                                <td className="harakat">
                                                    <div className="icon d-flex justify-content-center align-items-center ">
                                                        <Link to={tableItem?.tabName === "SUPER_VISOR" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/nazorat` : tableItem?.tabName === "GENERALLER" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/umumlashtiruvchi` : tableItem?.tabName === "FOR_DOING" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/bajarish` : tableItem?.tabName === "FOR_INFO" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/malumot` : tableItem?.tabName === "NOT_DONE" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/bajarilmagan` : `/kiruvchi_bajarish_ijro/${tableItem?.id}/radEtilgan`}
