...
export default function Bajarilmagan23kunKechContent({ itemsPerPage }) {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosInstance.get(`mainPage/lateTwoThreeDay/${localStorage.getItem('ids')}`, {
                headers: {
    ...
    const handlePageClick = async (e) => {
        try {
            const res = await axiosInstance.get(`mainPage/lateTwoThreeDay/${JSON.parse(localStorage.getItem('ids'))}?page=${e.selected}`, {
                headers: {
                    Authorization: "Bearer " + currentUser
                }
            })
    ...
    return (
        <div className="content mb-5">
            <h3 style={{ margin: "10px 0 0 20px", fontWeight: "bold", textTransform: "upperCase" }}>2-3 kun kechikkan</h3>
