...
export default function Bajarilmagan4kunKechContent({ itemsPerPage }) {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosInstance.get(`mainPage/lateMoreThanFourDay/${localStorage.getItem('ids')}`, {
                headers: {
    ...
    const handlePageClick = async (e) => {
        try {
            const res = await axiosInstance.get(`mainPage/lateMoreThanFourDay/${JSON.parse(localStorage.getItem('ids'))}?page=${e.selected}`, {
                headers: {
                    Authorization: "Bearer " + currentUser
                }
            })
    ...
    return (
        <div className="content mb-5">
            <h3 style={{ margin: "10px 0 0 20px", fontWeight: "bold", textTransform: "upperCase" }}>4 (~) kun kechikkan</h3>
