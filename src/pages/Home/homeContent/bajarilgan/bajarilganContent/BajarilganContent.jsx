...
export default function BajarilganContent({ itemsPerPage }) {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosInstance.get(`mainPage/allDone/${localStorage.getItem('ids')}`, {
                headers: {
    ...
    const handlePageClick = async (e) => {
        try {
            const res = await axiosInstance.get(`mainPage/allDone/${JSON.parse(localStorage.getItem('ids'))}?page=${e.selected}`, {
                headers: {
                    Authorization: "Bearer " + currentUser
                }
            })
