...
export default function VazifalarYangiContent({ itemsPerPage }) {
    ...
    useEffect(() => {
        const getData = async () => {
            const res = await axiosInstance.get(`mainPage/newTasks/${localStorage.getItem('ids')}`, {
                headers: {
    ...
    const handlePageClick = async (e) => {
        try {
            const res = await axiosInstance.get(`mainPage/newTasks/${JSON.parse(localStorage.getItem('ids'))}?page=${e.selected}`, {
                headers: {
                    Authorization: "Bearer " + currentUser
                }
            })
    ...
    const SearchData = async () => {
        ...
        try {
            const res = await axiosInstance.post(`` + JSON.parse(localStorage.getItem('ids')), {
                                    ...
                                    <tbody>
                                        {tableData?.content?.map((tableItem, index) => (
                                            <tr key={index}>
                                                ...
                                                <td className="text-center id">{tableItem.correspondentName}</td>
                                                <td className="text-center id">{tableItem.shortDescription}</td>
                                                ...
                                                <td className="harakat">
                                                    <div className="icon d-flex justify-content-center align-items-center ">
                                                        <Link to={tableItem?.tabName === "SUPER_VISOR" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/nazorat` : tableItem?.tabName === "GENERALLER" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/umumlashtiruvchi` : tableItem?.tabName === "FOR_DOING" ? `/kiruvchi_bajarish_ijro/${tableItem?.id}/bajarish` : `/kiruvchi_bajarish_ijro/${tableItem?.id}/malumot`}
